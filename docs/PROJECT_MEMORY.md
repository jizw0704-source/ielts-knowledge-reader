...
## V0.9.1 Progressive PWA Install Entry

- 今日推荐页新增渐进式 PWA 安装入口：只有浏览器触发 `beforeinstallprompt`、确认当前 PWA 可安装时，才显示“安装到设备”按钮。
- 点击按钮会在用户手势中调用浏览器安装确认窗口；接受、取消和异常情况都有明确反馈，并在一次调用后清理事件引用。
- 监听 `appinstalled`，安装完成后会隐藏安装入口并提示用户可从主屏幕打开。
- 从 standalone 模式启动时，今日页显示“已从主屏幕启动”，不再重复邀请安装。
- iOS Safari 和其他不支持 `beforeinstallprompt` 的浏览器不会显示无效按钮，继续使用现有手动“添加到主屏幕”说明。
- 安装说明已补充自动按钮与手动路径的关系，不改变基础离线能力、Service Worker 缓存策略或现有业务数据。
- 业务提交：`b1e1a6b Add progressive PWA install entry`。
- 本阶段业务改动只涉及 `script.js`，未修改 `index.html`、`style.css`、Manifest、Service Worker、文章、词典、查词规则或 `localStorage` 键。
- 静态校验通过：5 个 JavaScript 文件语法正常；12 篇文章、12 个语境词汇包和 286 条语境词汇通过内容校验。
- 375px 回退路径验收通过：当前应用内浏览器未提供安装事件，按钮按设计保持隐藏，手动说明完整可用且排版正常。
- 完整回归通过：阅读、点词、生词加入、刷新保留、熟悉程度、删除、文章筛选与打开、完成阅读、读后感和复制分享语正常；控制台无错误，测试数据已恢复。
- 当前环境未触发真实 Chromium 系统安装弹窗；需在真实 Android Chrome 或桌面 Chrome 上完成自动按钮路径的最终验收。

## 2026-08-14 项目状态复盘与下一阶段路线

### 当前结论

- 项目已经完成可公开访问、可安装、可基础离线使用的手机端 H5 MVP，核心阅读闭环可以正常使用。
- 当前阶段应从“把基础功能做出来”转向“让用户持续回来学习”。下一阶段的主要矛盾不再是页面或部署，而是每日内容的新鲜度和生词复习的学习闭环。
- GitHub `main`、远端 `origin/main` 与线上 GitHub Pages 已对齐到提交 `173bb16`；本次检查时工作区干净。

### 已验证的产品与内容基线

- 核心路径已完整：今日文章 -> 阅读文章 -> 点词释义 -> 加入生词本 -> 查看和管理生词 -> 查看文章库。
- 阅读辅助能力已具备：文章摘要、主题标签、阅读记录、完成状态、读后感和分享文案。
- 数据安全能力已具备：生词和阅读记录保存在当前浏览器，并支持 JSON 备份与恢复。
- 手机使用能力已具备：响应式 H5、Web App Manifest、Service Worker、基础离线缓存、iOS 手动添加到主屏幕说明，以及受浏览器能力控制的渐进式安装入口。
- 内容库现有 12 篇原创 IELTS-style 文章、8 条参考来源记录、12 个语境词汇包和 286 条语境词汇；内容校验全部通过，每篇文章约 755–900 词。
- 5 个 JavaScript 文件语法检查通过，线上 `script.js` 与本地版本校验值一致，最新 GitHub Pages 部署成功。

### 当前最重要的缺口

1. **“今日文章”已经失去每日新鲜度。** 当前逻辑在没有当天文章时总是回退到最新发布文章；内容库最新日期为 2026-07-27，因此之后每天都会显示同一篇 Peatlands 文章。
2. **生词本目前偏向收藏，尚未形成复习闭环。** 用户可以保存、标记熟悉程度和删除单词，但缺少一个短时、明确、有完成感的“今日复习”过程。
3. **内容供给仍然偏薄。** 12 篇文章足够验证 MVP，但不足以支撑长期每日阅读，需要稳定扩充和排期机制。
4. **维护风险正在累积。** `script.js` 已增长到约 2,561 行，且保留多套旧版和 V2 事件处理函数。现在不宜大规模重构，但继续加功能前需要先补关键行为测试，再做小范围清理。
5. **部分说明文档落后于实际产品。** `README.md` 仍将离线能力列为后续方向，项目规则中的阶段描述也没有完全反映 PWA 已上线的现状。
6. **真实设备验收仍未闭环。** 自动检查和浏览器回归不能代替 iPhone Safari、Android Chrome 的最终安装、主屏幕启动和离线体验确认。

### 推荐推进顺序

#### P0：发布收口与每日推荐修正

- 在真实 iPhone Safari 和 Android Chrome 上各完成一次安装、主屏幕启动、离线打开和本地数据保留验收。
- 先更新 `docs/PROJECT_RULES.md`，明确“今日文章”的产品规则，再实施确定性的每日轮换：当天有排期文章时优先展示；没有当天文章时，在已发布文章中按日期稳定轮换；同一天刷新结果保持一致；未来日期文章不得提前出现。
- 同步更新 `README.md` 与手机使用说明，使文档与已上线的 PWA 能力一致。

#### P1：落地“今日复习”学习闭环

- 在现有生词本中增加“今日复习”，第一版建议每次 10 个词，不增加新的底部导航。
- 单词卡先展示单词和原句，再揭示释义；用户选择“陌生 / 认识 / 掌握”，完成后给出进度和简单总结。
- 复用现有生词、语境和熟悉程度数据；如果新增复习日期或次数，必须保持旧数据和备份文件兼容。
- 暂不引入账号、后端、云同步或在线 AI，先验证用户是否愿意持续完成阅读与复习。

#### P2：建立持续内容供给

- 将内容库从 12 篇逐步扩充到至少 30 篇原创、来源可追溯的 IELTS-style 文章。
- 建立每周 2–3 篇的写作、人工审核、语境词汇补充、内容校验和发布日期排期流程。
- 保持当前版权边界：不内置、复制或改写真实 IELTS、Cambridge IELTS 或官方样题。

#### P3：补齐质量保障并控制技术债

- 先为今日文章选择、词典优先级、备份导入校验和 PWA 资源清单增加轻量自动化检查。
- 有测试保护后，再删除重复的旧版事件处理函数，并按职责逐步拆分 `script.js`；不做一次性大重构。
- 清理历史分支属于低优先级维护工作；如需删除分支，应单独确认范围后执行。

### 明确暂缓的方向

- 暂缓原生 App、微信小程序、登录、支付、后端、云同步、实时 AI 和自动采集。
- 在“每日内容 + 生词复习”能够形成稳定使用闭环前，不建议扩大平台复杂度。

### 建议立即开始的下一项

- 下一项建议定为 **V1.0-a 每日推荐轮换与发布规则**。它改动范围小，能直接修复首页长期重复同一篇文章的问题，也为后续 30 篇内容排期打基础。
- 因为这会调整产品行为，实施前先更新 `docs/PROJECT_RULES.md` 并获得确认；随后再修改业务代码、补检查、完成手机端回归并部署。
- 完成 V1.0-a 后，立即进入 **V1.0-b 今日复习**，把已经保存到生词本的词真正转化为可持续学习价值。

## 2026-08-29 V0.9.2 发布与 V1.0-a 每日推荐轮换

### V0.9.2 发布结果

- 每日分享海报版本已通过提交 `c7b35f3 Add daily share poster experience` 发布到 GitHub Pages。
- 线上包含每日首次展示、今日页重开入口、1080 × 1440 PNG 下载、系统分享回退、四张本地背景图、项目网址和二维码。

### V1.0-a 本地实现状态

- 新增确定性每日推荐：当天有排期文章时优先展示；没有排期时，在不晚于当天的已发布文章中按本地日期稳定轮换。
- 同一天刷新和页面切换结果保持一致；连续 12 个无排期日期覆盖现有 12 篇不同文章；未来文章不会提前出现。
- 早于第一篇文章发布日期时返回空状态，不新增 `localStorage` 键，也不修改文章、词典、生词或阅读记录数据。
- PWA 缓存版本更新为 `v1.0-a`，停止本地服务后仍可从缓存打开同一篇今日文章、文章库和海报入口。
- 本地检查通过：`script.js` 与 `sw.js` 语法、内容校验、差异格式、375px 与 390px 页面、当天排期优先、同日稳定、阅读页、文章库、海报入口、二维码和控制台状态。
- V1.0-a 的独立发布范围固定为 `README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_RULES.md`、`script.js` 和 `sw.js`，不得混入其他实验功能。

## 2026-08-30 V1.0-b 今日复习本地实现与验收

- 生词本新增“今日复习”入口，不增加新页面或底部导航；没有生词时继续显示原空状态。
- 每轮最多选择 10 个词，数量不足时全部进入；优先级固定为“陌生 -> 认识 -> 掌握”，同一天在同一熟悉度分组内保持稳定轮换。
- 复习卡先展示单词、来源文章和原文句子；用户查看释义后，才可选择“陌生 / 认识 / 掌握”并进入下一个词。
- 复用并持久化现有 `familiarity` 字段；没有增加 `localStorage` 键、数据字段或备份格式，复习过程仅保存在当前页面内存中，刷新后重新开始本轮。
- 完成页显示本轮三个熟悉度的数量，支持“再复习一次”和“返回生词列表”；中途退出会保留已经完成的熟悉度更新。
- 选择逻辑校验通过：0 个词返回空结果，7 个词全部进入，15 个词只取 10 个；同日结果稳定，次日同组顺序发生轮换，优先级符合规则。
- 375px 与 390px 手机宽度验收通过，无横向溢出；3 个测试词完整走通开始、揭示、三档评价、进度、总结、重新复习、退出和刷新保留，控制台无错误或警告。
- 离线验收通过：停止本地静态服务后仍能打开应用、生词本和今日复习入口，3 个测试词及熟悉度保持不变。
- PWA 缓存版本更新为 `v1.0-b`，并为 `style.css` 与 `script.js` 增加对应版本参数，避免发布后继续使用旧资源。
- V1.0-b 的独立发布范围固定为 `README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_RULES.md`、`index.html`、`script.js`、`style.css` 和 `sw.js`，不得混入文章、词典、查词规则或 MiniMax 实验功能。

## 2026-09-08 V1.0-c 品牌标志本地实现

- 用户从多轮概念中选定横向 `IELTS Knowledge Reader` 标准字方向，核心识别为突出显示的 `IELTS`、绿色阅读高亮线和阅读段落符号。
- 正式标志已重绘为纯矢量平面图形，不直接使用图像生成草稿，避免黑底、渐变、毛边和小尺寸文字失真。
- 品牌资源包括透明底横向主标志、透明底深色背景版、独立符号，以及对应 PNG 预览文件。
- PWA 图标从“阅读页面 + 绿色高亮行”符号提炼，提供 SVG、180px Apple Touch Icon、192px 和 512px PNG。
- 顶部品牌区接入横向标志，同时保留页面标题、元信息与阅读页返回按钮；手机窄屏下使用更紧凑的标志宽度。
- Manifest 增加 SVG 图标，Service Worker 缓存更新为 `v1.0-c` 并纳入全部品牌资源。
- 本阶段不修改文章、词典、查词规则、生词、阅读记录、备份格式、底部导航或 `localStorage` 键。
- 375px 首页与阅读页验收通过：横向主标志分别以 208px 和 152px 宽度显示，返回按钮、页面标题和难度信息无挤压或横向溢出。
- 390px 验收通过，页面宽度正常，SVG Favicon、品牌标志和 `v1.0-c` 样式资源均正确挂载，控制台无错误或警告。
- 离线验收通过：停止本地服务后刷新，顶部品牌标志、今日文章和底部导航仍可从 Service Worker 缓存显示。
- V1.0-c 的独立发布范围固定为品牌资源、PWA 图标、`index.html`、`style.css`、Manifest、Service Worker、README 与项目规则/记录；发布后必须核对 GitHub Pages 构建、线上资源版本和手机窄屏显示。

## 2026-09-08 双重价值产品定位

- 产品核心价值正式明确为“提高雅思阅读能力 + 增长跨学科知识”，对应主张为 `Read for IELTS. Learn for life.` / “为雅思而读，为认知而学”。
- 每篇正式文章必须分别通过阅读训练价值和知识增长价值两道门槛，不能以其中一项替代另一项。
- 新文章在写作前需要明确一个核心知识问题，并设计 2–4 个用户读后可以准确复述的知识收获。
- 内容选题优先覆盖科学、技术、环境、历史、社会、文化、心理与公共议题，并保持长期主题多样性。
- 本轮只更新产品定位、文章规范和内容生产流程，不修改界面、文章数据、词典、存储、PWA 缓存或用户数据。

## 2026-09-08 双重价值首页呈现

- 今日页主标题更新为“读懂英文，也读懂世界”，并展示 `Read for IELTS. Learn for life.` / “为雅思而读，为认知而学”。
- 首页产品说明明确为：每天一篇原创 IELTS-style 英文知识文章，在训练阅读能力的同时积累跨学科知识。
- 文章库标题、网页描述、PWA 描述和文章分享文案同步采用新的双重价值定位。
- `script.js` 资源版本和 Service Worker 缓存更新为 `v1.0-d`，确保发布后客户端可以获取新文案。
- 桌面端、390px 和 375px 首页验收通过；两种手机视口的页面宽度与视口一致，无横向溢出，控制台无错误。
- 基础离线刷新通过：新标题、双语主张、今日文章和品牌标志均可从缓存正常显示。
- 本轮未修改文章正文、摘要、词典、查词规则、生词、阅读记录、备份格式、底部导航、样式或 `localStorage` 键。
