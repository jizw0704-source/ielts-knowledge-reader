...
- “今日文章”在没有当天排期内容时会回退到最新发布文章；当前内容排期结束后可能连续多天显示同一篇文章。
- 暂无账号、后端和跨设备自动同步。
- 当前内容库只有 12 篇文章，尚不足以支撑长期每日阅读。
...
1. **V1.0-a 每日推荐轮换**：当天没有排期文章时，在已发布文章中按日期稳定轮换。
2. **V1.0-b 今日复习**：在生词本中增加每次约 10 个词的复习流程。
3. **持续内容供给**：逐步扩充到至少 30 篇原创、来源可追溯的文章。
4. **质量与维护**：补充关键自动检查后，再小范围清理重复事件处理逻辑和拆分 `script.js`。
