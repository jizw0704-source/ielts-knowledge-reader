...
一个同时提升雅思阅读能力与跨学科知识积累的手机端英文知识阅读器：每天读一篇原创 IELTS-style 知识文章，在语境中查词、收藏生词、完成短时复习，并真正理解一个值得知道的问题。

> **Read for IELTS. Learn for life.**
>
> 为雅思而读，为认知而学。

**在线使用：** [https://jizw0704-source.github.io/ielts-knowledge-reader/](https://jizw0704-source.github.io/ielts-knowledge-reader/)
...
```text
每日海报 → 今日文章 → 语境阅读 → 点词释义 → 加入生词本 → 今日复习 → 继续阅读
```

这条路径同时产生两种收获：一是对篇章结构、学术词汇和复杂句子的阅读训练；二是对科学、技术、环境、历史、社会与文化等主题的持续理解。每篇正式文章都必须同时通过“阅读训练价值”和“知识增长价值”两道质量门槛。

项目已经完成手机端 H5 MVP、PWA 基础离线能力、每日文章轮换和生词复习闭环。当前重点是扩大原创内容供给，并用真实手机和持续使用数据验证学习节奏。
...
| 后端与账号 | 暂无 |
| 当前阶段 | 核心学习闭环已完成，进入内容扩充与真实使用验证 |

## V1.0-d 双重价值定位

- 核心主张更新为 `Read for IELTS. Learn for life.` / “为雅思而读，为认知而学”。
- 首页明确呈现“一篇文章，两种收获”：训练雅思阅读能力，同时积累跨学科知识。
- 新文章需要分别通过阅读训练价值和知识增长价值两道门槛，并明确一个核心知识问题及 2–4 个可复述的知识收获。
- 网页描述、PWA 描述、文章库标题和分享文案已同步采用新的产品定位。

## V1.0-c 品牌更新

- 采用全新的 `IELTS Knowledge Reader` 横向品牌标志，突出 IELTS 阅读定位和绿色高亮识别。
- 新增透明底主标志、深色背景版和独立阅读高亮符号，提供 SVG 与 PNG 格式。
- PWA、Apple Touch Icon 和浏览器 Favicon 已统一使用新的阅读页面标志。
- 顶部品牌区在首页、文章库、生词本和阅读页保持一致，并适配 375px 与 390px 手机宽度。

## V1.0-b 更新重点

- **今日复习**：从现有生词中按“陌生 → 认识 → 掌握”优先级稳定选择最多 10 个词。
- **主动回忆**：先看单词、来源文章和原文句子，再主动揭示中英文释义与例句。
- **即时反馈**：每个词重新选择熟悉程度，结果立即保存在当前设备；完成后给出本轮统计。
- **延续使用**：支持重新复习、中途退出、刷新后的熟悉度保留，以及基础离线使用。
- **兼容原数据**：没有新增用户数据字段或存储键，原有生词和 JSON 备份格式保持兼容。
...
- 生词本内提供“今日复习”，每天按熟悉程度和日期稳定选择最多 10 个词。
- 复习时先显示单词和原文句子，再揭示释义；每次选择的熟悉程度会立即保存。
- 完成后显示本轮“陌生 / 认识 / 掌握”统计，并可重新复习或返回生词列表。
- 支持导出、校验并恢复 JSON 备份。
...
项目没有后端数据库，也不会把生词和阅读记录上传到服务器。当前使用三个本地存储空间：
...
- 今日复习的会话进度只保留在当前页面；刷新后会重新开始，但已经选择的熟悉程度不会丢失。
...
1. **P0 · 真实设备验收**：分别在 iPhone Safari 与 Android Chrome 验证安装、主屏幕启动、离线打开和本地数据保留。
2. **P1 · 持续内容供给**：逐步扩充到至少 30 篇原创、来源可追溯的文章；每篇文章先明确核心知识问题和 2–4 个知识收获，再进入写作、事实审核、词汇和排期流程。
3. **P2 · 复习节奏验证**：观察每天最多 10 词是否合适，再决定是否增加复习历史、间隔重复或提醒。
4. **P3 · 质量与维护**：先补每日推荐、复习选词、备份恢复和 PWA 资源的自动检查，再小范围清理重复逻辑和拆分 `script.js`。
